...
namespace ir {
    ...
    auto master_frame_counter_t::make(uint64 current) noexcept -> arc_ptr<self> {
        IR_PROFILE_SCOPED();
    ...
    auto master_frame_counter_t::current() const noexcept -> uint64 {
        IR_PROFILE_SCOPED();
    ...
    auto frame_counter_t::current_absolute() const noexcept -> uint64 {
        IR_PROFILE_SCOPED();
        return master().current();
    }

    auto frame_counter_t::current_relative() const noexcept -> uint64 {
        IR_PROFILE_SCOPED();
